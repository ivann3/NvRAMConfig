...
#include "../cfgfileinc/CFG_XXXX_File.h"      //这里把你添加的两个头文件包含进来
#include "../cfgdefault/CFG_XXXX_Default.h"
...
#include <stdio.h>
#include "CFG_MASSCOM_Warranty_File.h"
#include "CFG_MASSCOM_Warranty_Default.h"
#ifdef __cplusplus
extern "C"
{
#endif
    ...
    const TCFG_FILE g_akCFG_File_Custom[]=
    {
        ...
        { "/data/nvram/APCFG/APRDCL/MASSCOM_Warranty",   VER(AP_CFG_RDCL_FILE_MASSCOM_WARRANTY_LID), CFG_FILE_MASSCOM_WARRANTY_REC_SIZE,
            CFG_FILE_MASSCOM_WARRANTY_REC_TOTAL, SIGNLE_DEFUALT_REC  ,    (char *)&MASSCOM_Warranty_Default, DataReset , NULL
        },
	    {
          "/data/nvram/APCFG/APRDCL/File_Custom1",       VER(AP_CFG_CUSTOM_FILE_CUSTOM1_LID),         CFG_FILE_CUSTOM1_REC_SIZE,
            CFG_FILE_CUSTOM1_REC_TOTAL,                   SIGNLE_DEFUALT_REC,                                   (char *)&stCustom1Default,DataReset, NULL
        },
		{//这里应该是存储路径，未知是否必须用该路径      //这里是你加入Custom_NvRam_LID.h中的项。  //这里CFG_XXXX_File.h中得到每个File_XXXX_Struct的大小
          "/data/nvram/APCFG/APRDCL/XXXX",            VER(AP_CFG_XXXX_FILE_XXXX_LID),              CFG_FILE_XXXX_REC_SIZE,
            CFG_FILE_XXXX_REC_TOTAL,                   SIGNLE_DEFUALT_REC,                                   (char *)&stXXXXDefault,DataReset, NULL
		//CFG_XXXX_File.h中定义的结构体数量，一般为1个 //系统提供的默认值                                    //在CFG_Custom1_Default.h中赋的值，示例为1
        },
    };
    ...
    extern int iNvRamMasscomWarrantyLID = AP_CFG_RDCL_FILE_MASSCOM_WARRANTY_LID;
